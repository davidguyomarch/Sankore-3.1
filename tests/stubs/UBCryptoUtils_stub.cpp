...
{
    if (!sInstance)
        sInstance = new UBCryptoUtils(0);  // No UBApplication dependency
...
{
    EVP_CIPHER_CTX_cleanup(&mAesEncryptContext);
    EVP_CIPHER_CTX_cleanup(&mAesDecryptContext);
}
